Shader "Custom/HologramLitURP_EmissiveFixed"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                finalColor *= (flicker + scanline);

                float alpha = color.a * _Alpha;

                // Output color and emission in same pass
                float3 emission = _HologramTint.rgb * 2.0;
                return float4(finalColor + emission, alpha); // emission contributes to bloom
            }
            ENDHLSL
        }
    }
